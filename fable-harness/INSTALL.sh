...
for f in "$SRC_DIR"/.claude/hooks/fable_*.sh "$SRC_DIR"/.claude/hooks/fable_*.ps1; do
  copy_no_clobber "$f" "$TC/hooks/$(basename "$f")"
done
chmod +x "$TC"/hooks/fable_*.sh
...
if [ -f "$SETTINGS" ] && grep -q 'fable_guard_bash' "$SETTINGS"; then
  ...
  if [ ! -f "$SETTINGS" ]; then
    ...
    jq -s '
      ...
      | .hooks["Stop"]        = ((.hooks["Stop"]        // []) + $fab.hooks.Stop)
      | .hooks["PreCompact"]  = ((.hooks["PreCompact"]  // []) + $fab.hooks.PreCompact)
